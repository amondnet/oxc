...
const nodeVersions = require("fs")
  ...
  .trim()
  .split("\n");
envsVersions["node"] = nodeVersions.reverse().map((version) => {
...
nodeTests.forEach((test) => {
  let found = compatibilityTests.find((t) => t.name == test.name);
  if (!found) {
    return;
  }
  // Remove all node data from compatibilityTests then merge nodeTests into it.
  Object.keys(found.res).forEach((key) => {
    if (key.startsWith("node")) {
      found.res[key] = undefined;
    }
  });
  Object.assign(found.res, test.res);
});
